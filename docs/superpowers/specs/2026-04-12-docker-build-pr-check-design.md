...
- 当前基线下，Dockerfile 的 builder stage 会执行 `npm run build`，而 `package.json` 中的 `build` 实际映射到 `node scripts/build-sdk.mjs`；由于 Dockerfile 在该步骤前未复制 `scripts/`，构建会直接失败，导致新增 PR 检查无法落地。
- 本轮目标是在不改变现有 `release-image.yml` 语义的前提下，为 PR 新增一个独立 CI 检查，并补上使该检查可通过的最小 Dockerfile 变更，只验证镜像可以成功构建。
...
- 对 Dockerfile 做最小使能修复：仅复制 `npm run build` 所需的构建输入（已确认至少包含 `scripts/`），确保当前基线可以通过上述构建命令。
...
- 不为修复构建失败而重排 Dockerfile 阶段、重构镜像构建流程或扩大复制范围到与 `npm run build` 无关的文件。
...
- 同步修复 Dockerfile builder stage 的最小前置输入：在执行 `npm run build` 前补充复制 `scripts/`（以及若构建命令实际依赖的其他最小输入，仅限必要项），使 `node scripts/build-sdk.mjs` 在镜像构建上下文内可执行。
- 该 Dockerfile 变更仅作为新 PR 检查的最小使能条件存在，不改变发布流程职责、不引入额外构建逻辑，也不顺带做结构性清理。
...
- Dockerfile 修复仅限补齐 `npm run build` 缺失的构建输入，不借机扩大为通用容器构建重构。
...
- 评审 Dockerfile 时，确认新增复制内容仅覆盖 `npm run build` 必需输入，且已包含导致当前失败的 `scripts/`。
- 确认 Dockerfile 修复是为了让 `docker build --platform linux/amd64 -t auth-mini:pr-check .` 在当前基线下通过，而不是顺带修改发布语义。
...
- Dockerfile 已补齐 `npm run build` 缺失的最小构建输入，使当前基线下上述 Docker build 可以成功执行。
...
- 风险：为修复当前失败而顺手重构 Dockerfile，导致变更面超出批准范围。
  - 取舍：只补齐 `npm run build` 的必要输入，把 Dockerfile 修改控制在新 PR 检查的最小使能边界内。
