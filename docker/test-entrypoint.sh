...
python3 -c 'import subprocess, sys; cmd=["docker","build","--platform","linux/amd64","-t","auth-mini:test-entrypoint",sys.argv[1]]
try:
...
run_validation() {
  ...
  assert_file_contains "$dir/logs/auth-mini.argv" 'start /data/auth.sqlite --host 127.0.0.1 --port 7777 --issuer https://auth.example.com'
...
run_supervision() {
  local dir out status cid

  CURRENT_CASE='supervision auth exit propagates'
  dir=$(new_dir)
  write_auth_stub "$dir/stubbin"
  set +e
  out=$(run_once "$dir" -e AUTH_STUB_EXIT_EARLY=1 -e AUTH_STUB_EXIT_CODE=17 2>&1)
  status=$?
  set -e
  assert_status_eq "$status" 17
  assert_contains "$out" 'auth-mini early exit'
