---
description: "Use proactively for generation or exploratory work when the task might fail, need multiple attempts, involve many constraints, require verification, or produce intermediate findings that should not pollute the main Agent context. This skill runs a clean main-agent loop: the main Agent coordinates only, exports context to files, dispatches creator/reviewer SubAgents, keeps retries out of the main context, and receives only artifact paths plus PASS / RETRY / FAILED audit notes."
---
# Clean Agent
