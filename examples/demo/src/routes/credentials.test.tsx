...
describe('CredentialsRoute', () => {
  ...
  it('loads /me inside the route and renders credential tables', async () => {
        ...
        ed25519_credentials: [
          {
            ...
            name: 'Build runner',
            public_key:
              'MCowBQYDK2VwAyEAlongPublicKeyValueForTesting1234567890=',
    ...
    expect(
      await screen.findByRole('cell', { name: 'user@example.com' }),
    ).toBeInTheDocument();
    expect(
  ...
  it('shows a route-owned /me load error', async () => {
    ...
    sdkMocks.meFetch.mockRejectedValueOnce(
      new Error('Unable to load current account.'),
    );

    renderRoute();

    expect(
      await screen.findByText('Unable to load current account.'),
    ).toBeInTheDocument();
    expect(
      screen.queryByText('This account does not currently have a bound email.'),
    ).not.toBeInTheDocument();
    expect(
  ...
  it('reloads local /me after deleting a passkey', async () => {
    ...
    await user.click(
      await screen.findByRole('button', {
        name: 'Delete passkey first-passkey',
      }),
    );
  ...
  it('preserves delete success when follow-up /me refresh fails', async () => {
    ...
    sdkMocks.meFetch
      ...
      .mockRejectedValueOnce(
        new Error('Unable to refresh current account data.'),
      );
  ...
  it('keeps delete actions available for legacy tokens after refresh yields manageable amr', async () => {
    sdkMocks.sessionState.current = authenticatedSession(undefined, {
