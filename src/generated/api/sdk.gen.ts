...
/**
 * Read the current host and SQLite resource snapshot
 */
export const getAdminSystemResources = <ThrowOnError extends boolean = false>(options?: Options<GetAdminSystemResourcesData, ThrowOnError>) => (options?.client ?? client).get<GetAdminSystemResourcesResponses, GetAdminSystemResourcesErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/admin/resources',
    ...options
});
...
/**
 * Send an OTP to a new email address for the authenticated user
 */
export const startCurrentUserEmailChange = <ThrowOnError extends boolean = false>(options: Options<StartCurrentUserEmailChangeData, ThrowOnError>) => (options.client ?? client).post<StartCurrentUserEmailChangeResponses, StartCurrentUserEmailChangeErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/me/email/start',
    ...options,
    headers: {
...
/**
 * Verify an email-change OTP and update the authenticated user email
 */
...
/**
 * Delegate a downstream session from the authenticated main-site session
 *
 * Creates an additional session for the requested downstream audience. The caller must
 * present a valid main-site (issuer-audience) access token. The delegated session records
 * `sso` as its authentication method and carries the source authentication method together
 * with `sso` in its `amr` claim. Non-loopback targets cannot request the issuer audience.
 *
 */
export const authorizeSession = <ThrowOnError extends boolean = false>(options: Options<AuthorizeSessionData, ThrowOnError>) => (options.client ?? client).post<AuthorizeSessionResponses, AuthorizeSessionErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
